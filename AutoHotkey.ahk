...
/*
TODO: 
- Prevent Everything Esc from closing window if suggestions control is visible
- Fix Drag Explorer scroll
...
- Start Slack and Spotify in tray
- Change Explorer shortcuts to symlinks using mklink %1
- Close app windows when activated from Chrome
- Kiwi should minimize window when closed
- MPC Delete key should delete file
- Double click in certain apps should launch from Explorer process
  (or with explorer /s or /e on double click on folder)
    - ClassNN: Everything = SysListView321, SysListView322
    - ahk_class: Everything = EVERYTHING, µTorrent4823DF041B09
...
#IfWinActive, ahk_exe sublime_text.exe
    ...
    ; Return
#IfWinActive
#IfWinActive "AutoHotkey.ahk • (src)"
    ~^s::
        MsgBox,  Test
        Reload
#IfWinActive
...
; https://autohotkey.com/board/topic/50026-if-process-not-running-run-it/
; https://autohotkey.com/board/topic/102763-help-how-to-winactivate-without-specifying-window-title/
...
#w::
    ...
    WinGet, WindowPID, PID, ahk_id %Window%
    WinGet, ControlText, ControlList, ahk_id %Window%
    ControlHwnd := DllCall("WindowFromPoint", "int64", CursorX | (CursorY << 32), "Ptr")
    WinGet, ControlText, ControlList, ahk_id %Window%
...
Visible Text in Window:%WindowText%

Visible Text in Control:
%ControlText%
)
    Gui, Destroy
...
; Tags: drag explorer, debug explorer drag
; Author: aph
; Version: 0.3
...
$*MButton::
    ...
    AllowedApp := ahk_exe = "mmc.exe" or ahk_exe = "systempropertiesadvanced.exe" or ahk_exe = "filezilla.exe" or ahk_exe = "7zFM.exe" or ahk_exe = "uTorrent.exe"
    ; DisallowedClassNN = 
    DisallowedClass = ahk_class = "Shell_TrayWnd" or ahk_class = "WorkerW" or ahk_exe = "mpc-hc64.exe" or ahk_class = "VMUIFrame"
    AllowedClass := ClassNN = "DirectUIHWND3" or ClassNN = "DirectUIHWND4"or InStr(ClassNN, "SysTreeView32")
    DisallowedClassNN := ClassNN = "DirectUIHWND2" or ClassNN = "ListBox1"
    ; BypassApp := ahk_class = "MediaPlayerClassicW" or ahk_exe = "mpc-hc64.exe"
    IsToolbar := InStr(ClassNN, "ToolbarWindow") or InStr(ClassNN, "ReBarWindow") or InStr(ClassNN, "Edit") or InStr(ClassNN, "AddressBandRoot") or InStr(ClassNN, "statusbar") or InStr(ClassNN, "SysHeader") ; not (ClassNN) or ahk_exe = "mpc-hc64.exe"
    AllowedText := InStr(VisibleText, "Tree View") or InStr(VisibleText, "FolderView") or ahk_exe = "mpc-hc64.exe"
    ...
    If (Disallowed) {
         ; If (ShowTrayTip) {
             TrayTip, % "Specifically disallowed", %TrayTipText%
         ; }
    ...
    Else If (BypassApp) {
         ; If (ShowTrayTip) {
             TrayTip, % "Bypassing key to app", %TrayTipText%
         ; }
         SendInput, {MButton Down}
         WinActivate
    ...
    Else If (!AllowedApp) {
         ; If (ShowTrayTip) {
         ;     TrayTip, % "Not an allowed app", %TrayTipText%
         ; }
         SendInput, {MButton Down}
         Return
    }
    Else If (!AllowedText) {
         ...
         ;          . " `nAllowedText: " AllowedText
         ;     TrayTip, % "Not allowed text", %TrayTipText%
         ; }
         SendInput, {MButton Down}
         Return
    }
...
; Tags, wlorer, scroll right, scroll down,
...
; ; Activate Slack
; ; TODO: https://autohotkey.com/docs/misc/WinTitle.htm#ahk_exe
DetectHiddenWindows, On
#IfWinExist ahk_exe slack.exe
  ; ; doesn't activate unless button pressed
    #a::
         ; tooltip, % "Slack exists? " + WinExist("ahk_exe slack.exe")
...
; Restore Win+X key functionality Disallowed via Group Policy on CORE
if (A_ComputerName = "CORE") {
    #r::
         SetTitleMatchMode, 3
         IfWinExist, Run
...
#IfWinActive ahk_class ahk_class MPC-BE
