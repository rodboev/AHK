...
| file:///C:/Dropbox/Projects/AHK/herdr-new-window.py | Pipe proxy that opens an F10 window on a pre-made workspace |
...
| Esc | Alacritty active | Runs `herdr pane send-keys <pane> esc` and suppresses the physical key |
...
Other Claude Code defaults this stack already intercepts: Ctrl+Shift+C (`selection:copy`) is taken by Herdr for split_horizontal, and Ctrl+T (`app:toggleTodos`) is taken by Herdr for new tab. Ctrl+Shift+- (`chat:undo`) became available again once close tab moved to Ctrl+Shift+Backspace.

### 2. Codex and Claude compete with Herdr for the same keys

Codex's config sets `copy = "ctrl-shift-c"`. Herdr claims Ctrl+Shift+C for split_horizontal, so Codex never receives it inside Herdr. Same class of problem produced the earlier Ctrl+T fight: Herdr took Ctrl+T for a new tab, which stole Codex's transcript toggle, which is why Codex now uses Ctrl+O.
...
It uses `GetChildProcesses()` and `GetProcessCommandLine()` in file:///C:/Dropbox/Projects/AHK/processes.ahk, a Toolhelp snapshot plus a PEB read, not WMI. Measured at 17ms against WMI's 112ms per lookup, with identical results. `GetProcessCommandLine()` reads `RTL_USER_PROCESS_PARAMETERS.CommandLine` at offset 0x70 on x64, the same walk `GetProcessCwd()` does at 0x38 for the working directory.

Everything keyed on client detection depends on this: Escape, Ctrl+Tab, F10 inside a client, Win+E, and the admin and remote distinctions. Insert any wrapper between Alacritty and Herdr, a launcher script, a session-name shim, a virtualenv Python stub, and all of it silently falls back to passthrough. This is why the parent session removed the cmd and conhost wrappers from the F10 path.

Corollary: `[terminal] shell = { program = "herdr.exe" }` in Alacritty's config is load-bearing. Change it and the detection, plus the whole F10 prepared-window path, stop working.
...
file:///C:/Dropbox/Projects/AHK/AutoHotkey.ahk includes modules at lines 1101 to 1106 in this order: processes, terminal-anywhere, extended-spy, mbutton-drag, window-spawning, herdr-anywhere.
...
AutoHotkey intercepts Escape in Alacritty and calls the Herdr CLI directly, suppressing the physical key. Herdr's config has its own `esc` custom command that does the same thing. Both exist because they cover different cases.
...
Removing either one leaves a gap. The AutoHotkey path used to cost a hidden `cmd.exe` spawn per press. It now calls `pane.send_keys` over the API socket, about 15ms with no process spawn.
...
`AlacrittyWindowAtMouse()` in file:///C:/Dropbox/Projects/AHK/AutoHotkey.ahk (lines 940 to 956) exists because the window under the cursor sometimes reports as `herdr.exe` rather than `alacritty.exe`. It falls back to a geometric search over all Alacritty windows.
...
Changing the normalization character breaks either the doubled lines or the accidental submit. Both regressions happened during development.
...
AutoHotkey no longer decides whether a server is running, creates workspaces, or runs commands in panes. The helper owns all of that, which is why `HerdrAnywhereEnsureClient`, `HerdrAnywhereServerState`, `HerdrAnywhereWaitFocusedPane` and the whole `HerdrAnywhereRun` CLI wrapper are gone.

The Ctrl release wait exists because sending text while Ctrl is physically down produces literal `codex{Enter}` in the pane. terminal-anywhere.ahk solved the same problem the same way. That path types into the pane, so it keeps a 3 second reentrancy guard; two overlapping presses would interleave keystrokes. The guard cannot use `Try`/`Finally`, because inside a `Try` block AutoHotkey turns every ErrorLevel-setting command into a throw. It expires instead, so a press that dies mid-flight cannot wedge F10.
...
### 11a. Herdr's API socket, not the CLI
...
`HerdrApiLastError` carries the Win32 code: 2 means no server, 5 means the pipe belongs to a different token (an unelevated caller reaching for the admin session), 1460 means timed out. `HerdrAnywhereServerState()` treats 2, 3 and 5 as "nothing to reuse".

This replaced a `cmd.exe` plus `herdr.exe` spawn per call, writing through a temp file, roughly 37ms and two processes, with a 15ms pipe round trip and none. AutoHotkey now spawns no processes at all for Herdr except the helper itself.
...
- The relay uses overlapped I/O with independent read and write operations. An earlier version deadlocked when a blocking read starved writes, which froze typing, paste, and `workspace.focus`.
- `pipe_io()` confirms every zero-byte transfer with `GetOverlappedResult`, because an overlapped handle can complete a read synchronously and leave `lpNumberOfBytesRead` unset. `relay()` retries a bare zero three times, since a closed peer raises `ERROR_BROKEN_PIPE` instead. Without both, the relay dies while the window stays open and that window renders nothing from then on. Tests in file:///C:/Dropbox/Projects/AHK/tests/test_herdr_new_window.py
...
- It logs to `%TEMP%\AHK_Debug.log` as `herdr-new-window`, gated by `Debug.Log["herdr-new-window"]` and passed down as `--log <path>`. Each line is appended and the file closed again, because AutoHotkey truncates that log at startup and a held handle would block it. A fatal launch failure writes whatever the toggle says.

The real fix belongs in Herdr: attach a new client directly to a chosen workspace without focusing existing clients. That option does not exist in the installed build.

### 13. Window spawning is mostly delegated to DisplayFusion

`WindowSpawningEnabled := 1` with `WindowSpawningSpecialOnly := 1` in file:///C:/Dropbox/Projects/AHK/AutoHotkey.ahk (lines 37 and 38) limits the AutoHotkey module to Run dialogs, the Start and Search overlay, and owned `#32770` dialogs. New Alacritty windows are positioned by DisplayFusion, not by this script. If an F10 window lands on the wrong monitor, look at DisplayFusion first.

## Facts learned by testing, worth not rediscovering

**Newlines.** Thirteen encodings were tested. Only two worked in both Codex and Claude Code: `\u001B\r` (Escape then CR) and `\u001B[13;2u` (CSI-u Shift+Enter). CSI-u submits rather than inserting in Claude Code, so Shift+Enter uses `\u001B\r`. Raw `\n` queues in the console input buffer and flushes all at once on Enter, which looks like the newline was ignored until it suddenly is not. Ctrl+J works in Windows Terminal because Windows Terminal preserves it as a Win32 key event with the virtual key and modifier intact. Alacritty sends only byte `0x0A`, so the receiving program sees an LF with no Ctrl+J identity.
...
**Multi-window sizing.** One shared session across several windows means shared tab sizes. Herdr documents last-focus-wins; in practice the smallest window tends to win. Separate named sessions give independent sizing at the cost of losing a single unified workspace list. Upstream discussion 651 has several people asking for the same thing. The current setup keeps one shared session.

**Config reload behavior.** Herdr reloads its config live, and editing the file has been enough to fix keybindings that had gone stale after a restart. This produced several confusing episodes where Escape stopped and started working with no relevant code change. When something Herdr-side stops responding, touch its config or run `herdr server reload-config` before debugging anything else.
...
7. Enable `Debug.Log["herdr-anywhere"]` and `Debug.Log["herdr-new-window"]` in file:///C:/Dropbox/Projects/AHK/AutoHotkey.ahk (lines 29 and 30). Both write to `%TEMP%\AHK_Debug.log`, tagged by module name.
